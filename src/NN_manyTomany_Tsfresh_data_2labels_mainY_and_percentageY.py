...
def create_xy(train_path="train_data.csv.gz", test_path="test_data.csv.gz", use_gpu=False):
    ...
    features_to_ignore = ["pid", "fold", "gt_time", "percentage_ground_truth"] if "percentage_ground_truth" in train_data else ["pid", "fold", "gt_time"]
...
class MyNet(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.net = LSTMLayer(input_size=170, break_point=170,
...
# else:
#     X, Y, test_pids = extract_features("../data/processed/train_test_splits/10min_centered/train_tsfresh_data.csv.gz", "../data/processed/train_test_splits/10min_centered/test_tsfresh_data.csv.gz", use_gpu=True)
...
#     X, Y, test_pids = extract_features("train_data.csv.gz", "test_data.csv.gz", use_gpu=True)
    X, Y, test_pids = create_xy("../data/processed/train_test_splits/10min_centered/train_tsfresh_data.csv.gz", "../data/processed/train_test_splits/10min_centered/test_tsfresh_data.csv.gz", use_gpu=True)
    save_data(datafolder, X, Y, test_pids)
